...
def message_content(city_code,timestamps,info_time,news_list,sentence,cookie):
    content = (
        "****%s****\n"%datetime.strftime(info_time,"%Y-%m-%d %H:%M:%S")+
        "*************天气************\n\n"+
        weather_info(city_code,timestamps,cookie)+
        "\n\n*************热闻************\n\n"+
...
def wx_push(wxid,wxsecret,content):
...
def main_handler(event, context):
    ...
    wx_push(wxid,wxsecret,message_content(city_code,timestamps,info_time,get_news(news_type,news_time),get_sentence(),cookie))
